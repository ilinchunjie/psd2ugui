...
- Unity `2022.3` 或更高版本
- Windows Unity Editor，或 macOS Apple Silicon Unity Editor
- 已安装 Adobe Photoshop
- 可正常使用 TextMeshPro

说明：

- `Import PSD...` 仅支持 Windows 和 macOS Apple Silicon Unity Editor
- Intel Mac 当前不支持 `Import PSD...`
- PSD 导入流程强依赖 Photoshop 应用路径
- `Import Bundle...` 不依赖 Photoshop，但仍需要先准备好 bundle 目录
- macOS 首次导入时，系统可能要求允许 Unity 控制 Photoshop
...
- `Photoshop Application`
  Windows 传 Photoshop `.exe`；macOS 传 Photoshop `.app`
- `TextMeshPro Font`
...
```powershell
cargo run -p psd-export -- export .\your.psd --out .\out\demo --photoshop-path "C:\Program Files\Adobe\Adobe Photoshop 2025\Photoshop.exe"
```

在 macOS 上：

```bash
cargo run -p psd-export -- export ./your.psd --out ./out/demo --photoshop-path "/Applications/Adobe Photoshop 2025/Adobe Photoshop 2025.app"
```

兼容旧脚本时，也可以继续使用 `--photoshop-exe` 作为 `--photoshop-path` 的别名。

## 发布内置 CLI

Unity Package 需要同时携带 Windows 和 macOS Apple Silicon 两份 CLI：

- Windows：`com.miloooo.game.psd2ugui/Editor/Tools/Win/psd2ugui.exe`
- macOS Apple Silicon：`com.miloooo.game.psd2ugui/Editor/Tools/Mac/psd2ugui`
...
```powershell
cargo build -p psd2ugui --release
Copy-Item .\target\release\psd2ugui.exe .\com.miloooo.game.psd2ugui\Editor\Tools\Win\psd2ugui.exe -Force
```

2. 在 macOS Apple Silicon 构建：

```bash
cargo build -p psd2ugui --release
cp ./target/release/psd2ugui ./com.miloooo.game.psd2ugui/Editor/Tools/Mac/psd2ugui
chmod 755 ./com.miloooo.game.psd2ugui/Editor/Tools/Mac/psd2ugui
```

3. 确认 `Packages/com.miloooo.game.psd2ugui/Editor/Tools/Mac/psd2ugui` 已存在，再发布 package。

说明：

- 这台 Windows 开发机无法直接完成 macOS 链接，必须在 macOS 机器上产出最终二进制
- Unity 在 macOS 启动导入前会再尝试为内置 CLI 补执行权限

## 注意事项

- Unity UPM 安装时，这个仓库必须使用带 `?path=/com.miloooo.game.psd2ugui` 的 git URL
- `Unity Import Root` 不能直接设为 `Assets`，必须是 `Assets` 下的子目录
- `Import PSD...` 强依赖 Photoshop
- macOS 首次运行可能弹出 Automation 授权提示
